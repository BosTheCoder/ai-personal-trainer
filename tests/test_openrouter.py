...
def mock_user_settings():
    ...
    return UserSettings(
        user_id="test_user",
        goals=["fitness"],
        api_keys={"openrouter": "test_api_key"}
    )
...
async def test_send_prompt_rate_limit_retry(mock_user_settings, mock_response_success):
    ...
    with patch('backend.app.ai.openrouter.get_user_settings') as mock_get_settings:
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            mock_response_success_obj.json = AsyncMock(return_value=mock_response_success)
...
async def test_send_prompt_max_retries_exceeded(mock_user_settings):
    ...
    with patch('backend.app.ai.openrouter.get_user_settings') as mock_get_settings:
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            mock_response.raise_for_status = AsyncMock(side_effect=httpx.HTTPStatusError("Rate limited", request=None, response=mock_response))
...
async def test_send_prompt_http_error_retry(mock_user_settings, mock_response_success):
    ...
    with patch('backend.app.ai.openrouter.get_user_settings') as mock_get_settings:
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            mock_client.return_value.__aenter__.return_value.post = AsyncMock(
                side_effect=[httpx.RequestError("Connection failed"), mock_response_success_obj]
            )
...
@pytest.mark.asyncio
async def test_send_prompt_variable_substitution(mock_user_settings, mock_response_success):
    ...
    with patch('backend.app.ai.openrouter.get_user_settings') as mock_get_settings:
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            await send_prompt(
                "Create a {workout_type} workout for {duration} minutes targeting {muscle_group}",
                {
            ...
            expected_content = "Create a strength workout for 30 minutes targeting chest"
