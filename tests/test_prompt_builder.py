...
def mock_user_settings():
    ...
    return UserSettings(
        user_id="test_user", goals=["build muscle", "lose weight"], api_keys={}
    )
...
def sample_prompts():
    ...
    return {
        "single_workout": (
            "Create a UPDATED single workout for a user with the following "
            "preferences:\n\n- Goals: {goals}\n\n- Available equipment: "
...
class TestBuildSingleWorkoutPrompt:
    ...
    def test_basic_prompt_building(self, mock_user_settings, sample_prompts):
        ...
        with patch(
            "backend.app.services.prompt_builder.get_user_settings"
    ...
    def test_prompt_with_overrides(self, mock_user_settings, sample_prompts):
        ...
        with patch(
            ...
            mock_get_settings.return_value = mock_user_settings
            with patch.dict("main.prompts", sample_prompts):
                result = build_single_workout_prompt(
                    "test_user", constraints, overrides
                )
    ...
    def test_snapshot_fixed_inputs(self, sample_prompts):
        ...
        fixed_user_settings = UserSettings(
            user_id="snapshot_user", goals=["strength training"], api_keys={}
        )
        fixed_constraints = {
        ...
        with patch(
            ...
            with patch.dict("main.prompts", sample_prompts):
                result = build_single_workout_prompt(
                    "snapshot_user", fixed_constraints, fixed_overrides
                )
    ...
    def test_user_not_found_error(self, sample_prompts):
        ...
        with patch(
            ...
            with patch.dict("main.prompts", sample_prompts):
                with pytest.raises(
                    ValueError,
                    match="User settings not found for user: nonexistent_user",
                ):
    ...
    def test_missing_template_error(self, mock_user_settings):
        ...
        with patch(
            ...
            with patch.dict("main.prompts", {}):
                with pytest.raises(
                    ValueError, match="single_workout template not found in prompts"
                ):
    ...
    def test_empty_goals_default(self, sample_prompts):
        ...
        user_settings_no_goals = UserSettings(
            user_id="test_user", goals=[], api_keys={}
        )
        constraints = {"equipment": "dumbbells"}
    ...
    def test_empty_overrides_ignored(self, mock_user_settings, sample_prompts):
        ...
        with patch(
            ...
            with patch.dict("main.prompts", sample_prompts):
                ...
                result_whitespace = build_single_workout_prompt(
                    "test_user", constraints, "   "
                )
