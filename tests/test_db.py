...
from backend.app.db import (
    ...
    create_exercise_template_ref, get_exercise_template_ref, update_exercise_template_ref, delete_exercise_template_ref,
    create_workout, get_workout, update_workout, delete_workout,
    create_workout_plan, get_workout_plan, update_workout_plan, delete_workout_plan,
    create_hevy_sync_token, get_hevy_sync_token, update_hevy_sync_token, delete_hevy_sync_token
)
from backend.app.models import (
    UserSettings, Goal, ExerciseTemplateRef, Workout, WorkoutPlan, HevySyncToken
)
...
def test_exercise_template_ref_crud(temp_db):
    ...
    exercise_ref = ExerciseTemplateRef(
        template_id="push_up_template",
        name="Push-ups"
    )
...
def test_hevy_sync_token_crud(temp_db):
    ...
    sync_token = HevySyncToken(
        access_token="hevy_access_token_123",
...
def test_database_initialization(temp_db):
    ...
    try:
        ...
        expected_tables = [
            'usersettings', 'goal', 'exercisetemplateref', 
            'workout', 'workoutplan', 'hevysynctoken'
        ]
...
def test_nonexistent_records(temp_db):
    ...
    assert update_user_settings("nonexistent_user", UserSettings(user_id="test", goals=[], api_keys={})) is False
    assert update_goal("nonexistent_goal", Goal(name="test", description="test", template_id="test")) is False
