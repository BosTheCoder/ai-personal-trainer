...
from app.clients.hevy_client import HevyClient  # noqa: E402
from app.utils import (  # noqa: E402
    OTHER_NOTES_TEMPLATE,
    get_hevy_templates,
    match_exercise_template,
)
...
class TestExerciseMatching:
    ...
    async def test_pagination_handling(self):
        ...
        page1_templates = [
            {"id": f"template_{i}", "name": f"Exercise {i}"} for i in range(100)
        ]
        page2_templates = [
            {"id": f"template_{i}", "name": f"Exercise {i}"} for i in range(100, 150)
        ]
        ...
        assert client.get_exercise_templates.call_count >= 2
