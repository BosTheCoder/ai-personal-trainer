...
class TestGenerateWorkout:
    ...
    def test_build_workout_prompt_basic(self, sample_constraints):
        ...
        template = "Goals: {goals}, Equipment: {equipment}, Duration: {duration}, Level: {fitness_level}"
    ...
    def test_build_workout_prompt_with_overrides(self, sample_constraints, sample_overrides):
    ...
    async def test_generate_workout_success(self, sample_constraints):
        ...
        with patch("app.api.generate.send_prompt", new_callable=AsyncMock) as mock_send:
            with patch("app.api.generate.match_exercise_template", new_callable=AsyncMock) as mock_match:
                with patch("main.prompts", {"single_workout": "Template: {goals}"}):
                    mock_send.return_value = mock_ai_response
                    mock_match.return_value = {"template_id": "test_id", "name": "Test Exercise"}
    ...
    async def test_generate_workout_api_key_error(self, sample_constraints):
        ...
        with patch("app.api.generate.send_prompt", new_callable=AsyncMock) as mock_send:
            with patch("main.prompts", {"single_workout": "Template"}):
                mock_send.side_effect = ValueError("OpenRouter API key not found in user settings")
    ...
    @pytest.mark.asyncio
    async def test_generate_workout_with_overrides(self, sample_constraints, sample_overrides):
