...
from .models import ExerciseTemplateRef

_template_cache: Optional[List[Dict[str, Any]]] = None
_cache_timestamp: Optional[float] = None
CACHE_DURATION = 3600

OTHER_NOTES_TEMPLATE = {
    "template_id": "other_notes",
    "name": "Other – Notes"
}
...
async def get_hevy_templates(hevy_client: HevyClient) -> List[Dict[str, Any]]:
    ...
    if (_template_cache is not None and _cache_timestamp is not None and 
    ...
    while True:
        response = await hevy_client.get_exercise_templates(page=page, page_size=page_size)
...
async def match_exercise_template(name: str, hevy_client: Optional[HevyClient] = None, 
    ...
    try:
        ...
        for template in templates:
            ...
            similarity = difflib.SequenceMatcher(None, name_lower, template_name).ratio()
