...
class WorkoutOverrides(BaseModel):
    specific_exercises: Optional[List[str]] = Field(
        None, description="Specific exercises to include"
    )
...
def _suggest_weight_for_exercise(
    exercise_name: str, fitness_level: str
    ...
    level_weights = weight_suggestions.get(
        fitness_level.lower(), weight_suggestions["beginner"]
    )
    ...
    for line in lines:
        ...
        if re.match(r"^\d+\.", line) or line.startswith("**") or line.startswith("#"):
            ...
            rest_match = re.search(
                r"(\d+(?:-\d+)?)\s*(?:seconds?|mins?|minutes?)", line, re.IGNORECASE
            )
...
async def _apply_weight_suggestions(
    exercises: List[Dict[str, Any]], fitness_level: str
) -> List[Dict[str, Any]]:
    ...
    for exercise in exercises:
        ...
        weight_suggestion = _suggest_weight_for_exercise(
            exercise["name"], fitness_level
        )
...
def _build_workout_prompt(
    constraints: WorkoutConstraints,
    overrides: Optional[WorkoutOverrides],
    template: str,
) -> str:
    ...
    equipment_str = (
        ", ".join(constraints.equipment) if constraints.equipment else "bodyweight only"
    )
    ...
    if overrides:
        ...
        if overrides.avoid_exercises:
            avoid_list = ", ".join(overrides.avoid_exercises)
            formatted_prompt += f"\n\nPlease avoid these exercises: {avoid_list}"
...
async def generate_workout(request: GenerateWorkoutRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        formatted_prompt = _build_workout_prompt(
            request.constraints, request.overrides, template
        )

        ai_response = await send_prompt(
            prompt=formatted_prompt, variables={}, user_id=request.user_id
        )

        exercises = _parse_ai_workout_response(ai_response)

        enhanced_exercises = await _apply_weight_suggestions(
            exercises, request.constraints.fitness_level
        )

        workout = Workout(date=datetime.now(), exercises=enhanced_exercises)
        ...
        if "OpenRouter API key not found" in str(e):
            raise HTTPException(
                status_code=400, detail="OpenRouter API key not found in user settings"
            )
